...
| Its five sections that *are* owned carry very different coverage: `.text` 97.6%, `.data.rel.ro` 94.8%, `.data` 97.2%, **`.rodata` 40.7%**, `.init_array` 20.0% | razves' own report, rounded as it prints them |
...
absent from their indexes.

**Reading the members instead places them, and it is the default** (B-26). The estimate that deferred
it — "fifty times the bytes" — is true of the bytes and false of the time: both modes must inflate
the archive out of the klib first, and that inflate dominates. Measured on the same classpath, warmed:
**unplaced falls from 4,572,896 to 955,215 bytes, at 3,926 ms against 3,859 ms.** What remains is
`__unnamed_3673` at 142,096 bytes, generated by the linker for an anonymous object and in no archive
at all, and 175,682 bytes of Kotlin/Native runtime and libc that come from the compiler distribution
rather than from any klib.
