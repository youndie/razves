...
class RefusalsTest {
    ...
    private fun klibs(targets: List<String>) =
        PackageToModule(listOf(Klib("io.ktor:ktor-http", targets, setOf("io.ktor.http"))))
    ...
    fun anElfBinaryAgainstAppleKlibsIsRefused() {
        val failure =
            assertFailsWith<IllegalArgumentException> {
                Attribution.report(ElfReader.read(elf(), "app.kexe"), modules = klibs(listOf("macos_arm64")))
            }
    ...
    fun anElfBinaryAgainstLinuxKlibsIsAccepted() {
        val report = Attribution.report(ElfReader.read(elf(), "app.kexe"), modules = klibs(listOf("linux_x64")))
    ...
    fun anElfX86BinaryIsEitherLinuxOrAndroidAndNeitherIsRefused() {
        ...
        val report = Attribution.report(ElfReader.read(elf(), "app.kexe"), modules = klibs(listOf("android_x64")))
        assertTrue(report.hasModuleAttribution)
    }
    ...
    fun aMachOBinaryKnowsItIsMacOsFromItsBuildVersion() {
        ...
        val report = Attribution.report(image, modules = klibs(listOf("linux_x64")))
        assertTrue(report.hasModuleAttribution, "razves cannot name this target, so it claims nothing about it")
    }

    @Test
    fun klibsThatNameNoTargetAtAllRefuseNothing() {
        // A metadata-only klib declares no `native_targets`. That is not a contradiction with
        // anything; it is an absence of evidence.
        val report = Attribution.report(ElfReader.read(elf(), "app.kexe"), modules = klibs(emptyList()))
        assertTrue(report.hasModuleAttribution)
    }
